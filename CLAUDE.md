# Claude Code Project Instructions — zstd-kotlin

## Project Overview

This is **zstd-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`zstd`](https://crates.io/crates/zstd).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/zstd/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/zstd>
